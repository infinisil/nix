...
string showType(ValueType type)
{
    switch (type) {
        ...
        case tValue: return "a reference to another value";
    }
    abort();
}
...
void Expr::evalMinimal(EvalState & state, Env & env, Value & v)
{
    abort();
}
...
void ExprLazyBinOp::evalMinimal(EvalState & state, Env & env, Value & v)
{
    abort();
}
...
void ExprInt::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v = this->v;
}

void ExprFloat::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v = this->v;
}

void ExprString::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v = this->v;
}

void ExprPath::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v = this->v;
}

void ExprAttrs::evalMinimal(EvalState & state, Env & env, Value & v)
{
...
void ExprLet::evalMinimal(EvalState & state, Env & env, Value & v)
{
    ...
    body->evalMinimal(state, env2, v);
}

void ExprList::evalMinimal(EvalState & state, Env & env, Value & v)
{
...
void ExprSelect::evalMinimal(EvalState & state, Env & env, Value & v)
{
...
void ExprOpHasAttr::evalMinimal(EvalState & state, Env & env, Value & v)
{
    Value vTmp;
    Value * vAttrs = &vTmp;
...
void ExprLambda::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v.type = tLambda;
    v.lambda.env = &env;
    v.lambda.fun = this;
...
void ExprApp::evalMinimal(EvalState & state, Env & env, Value & v)
{
    /* FIXME: vFun prevents GCC from doing tail call optimisation. */
    Value vFun;
    e1->eval(state, env, vFun);
    state.callFunctionMinimal(vFun, *(e2->maybeThunk(state, env)), v, pos);
}
...
{
    callFunctionMinimal(fun, arg, v, pos);
    // TODO: This forceValue also calls another minimal eval within, problably a small abstraction mismatch
    forceValue(v, pos);
}

void EvalState::callFunctionMinimal(Value & fun, Value & arg, Value & v, const Pos & pos)
{
    ...
    if (fun.type == tAttrs) {
      ...
      if (found != fun.attrs->end()) {
        ...
        return callFunctionMinimal(v2, arg, v, pos);
      }
    }
    ...
    if (loggerSettings.showTrace.get())
        try {
            lambda.body->evalMinimal(*this, env2, v);
        } catch (Error & e) {
        ...
        fun.lambda.fun->body->evalMinimal(*this, env2, v);
}
...
void ExprWith::evalMinimal(EvalState & state, Env & env, Value & v)
{
...
void ExprIf::evalMinimal(EvalState & state, Env & env, Value & v)
{
    (state.evalBool(env, cond, pos) ? then : else_)->evalMinimal(state, env, v);
}

void ExprAssert::evalMinimal(EvalState & state, Env & env, Value & v)
{
    ...
    body->evalMinimal(state, env, v);
}

void ExprOpNot::evalMinimal(EvalState & state, Env & env, Value & v)
{
    mkBool(v, !state.evalBool(env, e));
}

void ExprOpEq::evalMinimal(EvalState & state, Env & env, Value & v)
{
...
void ExprOpNEq::evalMinimal(EvalState & state, Env & env, Value & v)
{
    Value v1; e1->eval(state, env, v1);
    Value v2; e2->eval(state, env, v2);
    mkBool(v, !state.eqValues(v1, v2));
...
void ExprOpAnd::evalMinimal(EvalState & state, Env & env, Value & v)
{
    mkBool(v, state.evalBool(env, e1, pos) && state.evalBool(env, e2, pos));
}

void ExprOpOr::evalMinimal(EvalState & state, Env & env, Value & v)
{
    mkBool(v, state.evalBool(env, e1, pos) || state.evalBool(env, e2, pos));
}

void ExprOpImpl::evalMinimal(EvalState & state, Env & env, Value & v)
{
    mkBool(v, !state.evalBool(env, e1, pos) || state.evalBool(env, e2, pos));
}
...
void ExprOpUpdate::evalMinimal(EvalState & state, Env & env, Value & v)
{
    v.type = tLazyBinOp;
...
void ExprOpConcatLists::evalMinimal(EvalState & state, Env & env, Value & v)
{
    Value v1; e1->eval(state, env, v1);
    Value v2; e2->eval(state, env, v2);
    Value * lists[2] = { &v1, &v2 };
...
void ExprConcatStrings::evalMinimal(EvalState & state, Env & env, Value & v)
{
    PathSet context;
    std::ostringstream s;
    NixInt n = 0;
...
void ExprPos::evalMinimal(EvalState & state, Env & env, Value & v)
{
    state.mkPos(v, &pos);
}
